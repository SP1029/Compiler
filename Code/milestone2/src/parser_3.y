...
Function_Defination_Semantic_Actions:
    %empty
    {
        if (current_symbol_table->name == "__init__" && current_symbol_table->parent_symbol_table->type==CLASS_SYMBOL_TABLE){
            // int alloc_size = current_symbol_table->parent_symbol_table->current_offset;
            // current_symbol_table->symbol_table["self"]->assigned_temporary = gen_3ac_function_call(alloc_mem_arg , {make_constant_arg(alloc_size)});
            // for(auto &it:current_symbol_table->function_parameters){
            //     if (it!="self") current_symbol_table->symbol_table[it]->assigned_temporary = gen_3ac_command_res("popparam");
            // }
            for(auto &it:current_symbol_table->function_parameters){
                (it.second)->assigned_temporary = gen_3ac_command_res("popparam");
            }
        }
        else{
            for(auto &it:current_symbol_table->function_parameters){
...
|   PRINT '(' Test ')'
    {
        gen_3ac_function_call(print_arg , {$<res_arg>3} , 1);
    }
Expression: 
...
Test: 
    or_Test 
    {
        $<res_arg>$ = $<res_arg>1;
        // Type Cast
        $<res_arg>$ = gen_3ac_convert($<res_arg>$, $<node_ix>$);
    }
...
|   Atomic_Expression Trailer
    {
        ...
        switch ($<trailer_type>2)
        {
            case '(':
            {
                ...
                if ($<is_class>1)
                {
                    ...
                    if ($<is_func>1==false)
                    {
                        ...
                        $<res_arg>$ = gen_3ac_function_call(alloc_mem_arg , {make_constant_arg(alloc_size)} , 1);
                        gen_3ac_command_arg("param" , $<res_arg>$);
                        gen_3ac_function_call(fptr->func_pointer_arg , $<res_arg_list>2 , $<res_arg_list>2.size()+1);
                    }
                ...
                else
                {
                    ...
                    {
                        gen_3ac_command_arg("param" , $<res_arg>1);
                        $<res_arg>$ = gen_3ac_function_call(fptr->func_pointer_arg , $<res_arg_list>2 , $<res_arg_list>2.size()+1);
                    }
                    // Normal Function
                    else
                    {
            ...
            case '[':
            {
                ...
                temp = gen_3ac_arithmetic(temp , make_constant_arg(4) , "+");
...
|   '[' Test_List ']' 
    {
        ...
        $<res_arg>$ = gen_3ac_function_call(alloc_mem_arg , {make_constant_arg(list_element_size*num_elements+4)} , 1);
        gen_3ac_mov_to_reference(make_constant_arg(num_elements) , $<res_arg>$);

        instr_arg* temp = gen_3ac_arithmetic($<res_arg>$ , make_constant_arg(4) , "+");
...
|   Tests ',' Test
    {
        ...
        {
            cout<<$<data_type>1.second<<" "<<$<data_type>3.second<<'\n';
